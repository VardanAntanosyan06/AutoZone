...
const Login = async (req,res)=>{
  try {
    const {pin,phoneNumber,token} = req.query;
    
    if(token){  
      const id = jwt.verify(token, process.env.JWT_SECRET).user_id;
    ...
    if(!phoneNumber || !pin) return res.status(403).json({message:"phoneNumber or pin cannpot be empty"})
    const User = await Users.findOne({where:{phoneNumber}})

    if(User && (await bcrypt.compare(User.pin,pin))){
      return res.json({success:true,User})
    }
